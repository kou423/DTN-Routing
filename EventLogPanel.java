...
public class EventLogPanel extends JPanel 
	...
	public void hostsConnected(DTNHost host1, DTNHost host2) {
		processEvent(conUpCheck, "接続しました", host1, host2, null);
	}

	public void hostsDisconnected(DTNHost host1, DTNHost host2) {
		processEvent(conDownCheck, "接続を解除しました", host1, host2, null);
	}

	public void messageDeleted(Message m, DTNHost where, boolean dropped) {
		if (!dropped) {
			processEvent(msgRemoveCheck, "メッセージの意図的廃棄", where, null, m);
		}
		else {
			processEvent(msgDropCheck, "メッセージの自然廃棄", where, null, m);
		}
	}
	...
	public void messageTransferred(Message m, DTNHost from, DTNHost to,
		...
		if (firstDelivery) {
			processEvent(msgDeliveredCheck, "メッセージ到達", from, to, m); 
		}
		else if (to == m.getTo()) {
			processEvent(msgDeliveredCheck, "メッセージをもう一度配達", 
					from, to, m);
		}
		else {
			processEvent(msgRelayCheck, "メッセージ中継", from, to, m);
		}
	}

	public void newMessage(Message m) {
		processEvent(msgCreateCheck, "メッセージ作成", m.getFrom(), null, m);
	}
	
	public void messageTransferAborted(Message m, DTNHost from, DTNHost to) {
		processEvent(msgAbortCheck, "メッセージの中継中止", from, to, m);
	}
	
	public void messageTransferStarted(Message m, DTNHost from, DTNHost to) {
		processEvent(msgTransferStartCheck,"メッセージの中継開始", from,
				to,m);
