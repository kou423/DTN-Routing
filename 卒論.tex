...
%%=============第１章=============
\chapter{序論}
近年，インターネットは，情報交換・共有システムの一部として，社会・経済のインフラストラクチャの役割を担っている．しかし，現代においても，世界の全ての地域に均一な通信サービスを提供することは，コストが掛かる上に無線帯域の制約やインフラ整備の課題があるため困難である．また，情報格差が深刻な地域のインフラ構築や，災害時等に本来の通信インフラの機能や性能が低下・停止した場合にも利用出来る情報通信サービスの実現が必要とされている．
...
DTNの概念はビントン・サーフ氏によって提案された惑星間インターネットの構想が基になっている．その惑星間インターネットには2つの大きな問題点が挙げられる．まず，惑星間というのは光速で進む電波であっても，届くのに長い時間を要する．次に，通信機が惑星によって見えなくなり，その間，通信が途絶える．これらの問題をそれぞれ，「通信遅延」，「通信途絶」として研究されてきた[2]．また，これら2つの問題は，「時間的不連続性」と「空間的不連続性」における問題であると考えることも出来る．地上におけるノード間通信も然り，冒頭に記した，「災害時等に本来の通信インフラの機能や性能が低下・停止した場合にも利用出来る情報通信サービスの実現」と言う点で2つの問題をDTNとして適応させることが考えられた．

DTNの特徴的な技術として，蓄積運搬形転送がある．この転送手法には，電子メールのような非同期通信による時間的不連続性と，それにより即時性を要求しないことから，常時接続性を必要としない空間的不連続性が存在する．また，ノード間のデータ送受信には，再開を待つ，あるいは最適なタイミングを待つための一時的「蓄積」と，物理的に車や列車を用いて情報を運ぶ「運搬」を用いる必要がある．そこで，無線通信の距離（干渉距離）を短くすることで，大容量のデータ転送を無線資源を浪費することなく実行することが可能になる．そのような人や車等における蓄積運搬形転送の中で，モバイルアドホック網における感染形中継転送方式（以降，Epidemic Routing）が確率的中継転送方式の最も代表的な方式として存在する．Epidemic Routingでは，ノードの移動や遭遇，送信を繰り返すことで全てのノードにデータを流通させる[1][3]．結果的に伝染病のように広がり，いずれかのメッセージが宛先ノードに到達することになる．しかし，Epidemic Routingではメッセージの到達を引き換えに，ノードのバッファに多くの複製メッセージが残存してしまう．そのため，Recovery Scheme（回復手法）やAnti-Packet（残存メッセージ除去パケット）を使用したルーティング手法等が提案されてきた[1]．
...
\subsection{本研究で用いる仮定}
本研究では，災害時等で基地局を介さないノード間通信を行う．ノードは人と車に分類されており，別々の進路を移動して行く中ですれ違うノードとBluetooth（10m以内を通信エリア）を用いて，メッセージの送受信を行う．各ノードには自身で生成したメッセージや他ノードから受け取ったメッセージを一定容量のストレージに保持する．従来手法と異なる点として，すれ違ったノードのリスト化を行い，それを基に新たにTTLを設けると言う点である．

現実においての各ノードは，バッファの空き容量が異なっていたり，端末のスペックに大きな差が存在したりするが，それらを忠実に再現することは困難である．そのため，各ノードには，ある一定の端末環境を用意し，必要最低限の機能のみを備える．

\subsection{問題の定式化}
本研究は，3.1の条件を基に，一定時間経過後に算出される「データ到達率」，「メッセージ平均遅延時間」，「メッセージ削除数」を評価指標とする．この3つの評価指標を従来手法の結果と比較し，全ての項目において改善されるように努める．

しかし，目指す先は全ての項目においての改善だが，理論上では困難である．例えば，Epidemic RoutingとMax Propを例に挙げて考える．Epidemic Routingでは，メッセージを複製し続ける特性があるが，このルーティングにおいては，届け終えたメッセージを保有する各ノードのバッファから，残存メッセージを削除するためのメソッドが存在しない．そのため，TTLによるタイムアウトやバッファ溢れによりメッセージ削除数は多くなってしまう．一方のMax Propにおいても，優先度の低いメッセージを意図的に削除を行う特性から，必然的にメッセージ削除数は多くなってしまう．そのため，どれか1つの評価指標の改善を行うと，別の評価指標において，劣る結果となってしまう．少なくともこの2つのルーティングに共通することは，各ノードが自身の判断で削除するメッセージを選択してしまっている点である．私たちは，その点に問題があるのではないかと考えた．

これらの問題認識から，各ノードがそれぞれ勝手に決めたメッセージを削除するよりも，通信を行う他ノードが，過去にすれ違ったノードを提示することで，到達する可能性の高いメッセージをバッファに残し，宛先に届けることが出来るのではないかと考えた．本研究では，このような外的要因を考慮した，新たなルーティング手法を考案し，提案手法として第4章に示す．
...
ベースTTL300における，ノード数を100，200，300に設定したときの，「データ到達率」，「メッセージ平均遅延時間」，「メッセージ削除数」の結果を以下に示す．ここで，MaxPropの削除数については，ベースTTLによるタイムアウトやバッファ溢れ等によるメッセージの削除(dropped)だけでなく，MaxPropのアルゴリズムによるメッセージの削除(removed)についての両方を記録し，表には「re」として記録している．
%%==============================================
\begin{figure}[h]
\centering
\includegraphics[width=80mm]{figures/default_TTL300/default_TTL300_toutaturitu.png}
\caption[]{データ到達率(疎状態マップ \textit{TTL300})}
...
\caption[]{メッセージ平均遅延時間(疎状態マップ \textit{TTL300})}
...
\caption[]{メッセージ削除数(疎状態マップ \textit{TTL300})}
...
\begin{table}[H]
	\begin{center}
			 \caption[]{優先TTL(疎状態マップ ベース\textit{TTL300})}
			 \label{priorityTTL_somitsu300}
			 \begin{tabular}{|c|c|}
 \hline
 ノード数&優先TTL[\textit{min}]\\
 \hline
 100&410\\
 \hline
 200&290\\
...
\caption[]{データ到達率(疎状態マップ ベース\textit{TTL720})}
...
\caption[]{メッセージ平均遅延時間(疎状態マップ ベース\textit{TTL720})}
...
\caption[]{メッセージ削除数(疎状態マップ ベース\textit{TTL720})}
...
\begin{table}[H]
	\begin{center}
			 \caption[]{優先TTL(疎状態マップ ベース\textit{TTL720})}
...
\caption[]{データ到達率(密状態マップ ベース\textit{TTL300})}
...
\caption[]{メッセージ平均遅延時間(密状態マップ ベース\textit{TTL300})}
...
\caption[]{メッセージ削除数(密状態マップ ベース\textit{TTL300})}
...
\begin{table}[H]
	\begin{center}
			 \caption[]{優先TTL(密状態マップ ベース\textit{TTL300})}
...
\caption[]{データ到達率(密状態マップ ベース\textit{TTL720)}}
...
\caption[]{メッセージ平均遅延時間(密状態マップ ベース\textit{TTL720})}
...
\caption[]{メッセージ削除数(密状態マップ ベース\textit{TTL720})}
...
\begin{table}[H]
	\begin{center}
			 \caption[]{優先TTL(密状態マップ \textit{TTL720})}
 ...
 最初の想定では，優先TTLを初期TTLよりも大きく設定することで，データ到達率の向上を見込んでいたが，実際には優先TTLを
 ベースTTLよりも小さくすることで，データ到達率が向上した．これにより，かえってメッセージ削除数が増加したのだと考えrareru．
 優先TTLがベースTTLよりも小さく設定した場合において，効果が得られた原因として，初期TTL自体既に大きすぎてしまっていて，それを優先TTLにより小さくしたことで，
適切なTTLとなり，データ到達率が向上した可能性がある． 
\newpage
\chapter{結論}
...
また、本研究における提案手法を現実の災害時に役立てるためには，各地域のマップ形状や端末保有台数等を考慮しなければならない．よって，災害地域の状況を正確に把握することが，この提案手法が最大限に効果を発揮するために必要不可欠であり、
状況把握のための方策や指標についても検討する必要がある。
